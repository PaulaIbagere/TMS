...
function GetNoOFStudent($CollegeCode1,$uncode1,$DeptNo,$AcadDeg,$Sem,$Classno,$MaxYear,$SecID)
{
	...
	$result31=mysql_query($sql_query31);
...
{
	$conn = db_connect();
	$results = mysql_query("select AcadDegreeId from Departments where 
								UniversityCode='$uncode' and 
								CollegeCode='$CollegeCode' and 
								DeptNo='$DeptNo'");
	$rows=mysql_fetch_row($results);
...
function GetNumberOfClasses($AcadProgId)
{
	...
	$results = mysql_query("select NoOfSemester from AcadDegree where AcadDegreeId='$AcadProgId'");
...
function GetSectionName($uncode,$CollegeCode,$DeptNo,$AcadProgId,$ClassNo,$SecID)
{
	...
	$result=mysql_query("select SecName from DeptSection where 
    		 	UniversityCode='$uncode' and CollegeCode='$CollegeCode' and 
...
function GetCollegeTimeSlot($uncode1,$CollegeCode1,$Sem,$year)
{
	...
	{
		$trow=mysql_fetch_row($result6);
		
		//get Time Slots
		$res6=mysql_query("select * from TimeSlots where TSID='$trow[0]'");
		$slot=mysql_fetch_row($res6);
				...
				if(($op==1)&&($s==1))
				{
				  if(strcmp($NoStud,"")!=0)
				  {
			    	...
			    	<?php
					 ...
					 {
					 	$result33=mysql_query("select BId,SubBId from  usedby  where 
					 							AcadYNo='$MaxYear' and 
					 							UniversityCode='$uncode1' and 
					...
					if(strcmp($CapGroup,"")!=0)
			   		{
						...
						$results=mysql_query($sqls_query);

						if(mysql_num_rows($results))
						{
						  while($rowss=mysql_fetch_row($results))
						   {
						   //(2) select the Groups Inserted
							...
							SemNo='$Sem' and
							ClassNo='$Classno' and SecID='$SecID'";
							$result88=mysql_query($sql_query88);
							if (mysql_num_rows($result88))
							{
							  while($row88=mysql_fetch_row($result88))
							   {
		...
		?>
			<select size="1" name="D4" tabindex="5" style="font-size: 12pt; color:#003366; font-family:Traditional Arabic; font-weight:bold" dir="rtl" onchange="javascript:document.fmanage.action='DeptManage.php?AcadDeg=<?php echo($AcadDeg);?>&Class=<?php echo($Classno);?>&Sem=<?php echo($Sem);?>&uncode=<?php echo($uncode1);?>&CollegeCode=<?php echo($CollegeCode1);?>&Dept=<?php echo($DeptNo);?>&op=<?php echo($op);?>&s=<?php echo($s);?>#otherDept';javascript:document.fmanage.submit();">
				...
				<?php
					...
					$results7=mysql_query($sqls_query7);
					if (mysql_num_rows($results7)>0)
					{
					  while($rows7=mysql_fetch_row($results7))
					  {
						...
						// check SubCode store on ManagingLec Table
							...
							$results77=mysql_query($sqls_query77);

						    $rows77=mysql_fetch_row($results77);
							...
							// if not inserted
								...
								$resultsd=mysql_query($sqls_queryd);
...
function sub_Form($uncode1,$CollegeCode1,$AcadDeg,$DeptNo,$Classno,$Sem,$op,$s,$year,$LectureName,$mday,$msub,$mteach,$GId,$SecID)
{
	...
	if (mysql_num_rows($result11))
	{
		...
		$aresults=mysql_query($asqls_query);

		$arows=mysql_fetch_row($aresults);
...
function CheckTime($BId,$year,$LectureName,$mday,$avTime)
{
	...
	$Mang_query = "select count(MTimes) from ManagingLec where
		...
		BId='$BId' and
		SubBId='$LectureName'";
    $Mresult=mysql_query($Mang_query);
...
function CheckTeacher($uncode1,$CollegeCode1,$year,$mday,$mteach,$avTime,$Sem)
{
	...
	else
	{
    	...
    	//CollegeCode='$CollegeCode1' and
    	$Mresults=mysql_query($Mang_querys);

		$mrows=mysql_fetch_row($Mresults);
		if ($mrows[0]==0)
...
function CheckSubjectType($year,$mday,$avTime,$LectureName,$Sem)
{
	...
	$Mresult=mysql_query($Mang_query1);
...
function DisplayDetails($BId,$year,$LectureName,$mday,$avTime,$Sem)
{
	// Display contains:
	// CollegeName:
	// ClassNo:
...
function FinalDetails($year,$mday,$avTime,$uncode1,$CollegeCode1,$DeptNo,$AcadDeg,$Classno,$Sem,$f,$SecID)
{
	// Display contains:
	// SubjectName:
			//> if(GId==0): Lecture
...
//
function FinalReports($year,$mday,$avTime,$uncode1,$CollegeCode1,$DeptNo,$AcadDeg,$Classno,$Sem,$SecID)
{
...
function DeleteDetails($year,$mday,$avTime,$uncode1,$CollegeCode1,$DeptNo,$AcadDeg,$Classno,$Sem,$TeacherId,$f,$SecID)
{
	// Display contains:
	// SubjectName:
			//> if(GId==0): Lecture
	 			...
	 			<tr>
						...
						<?php
							...
							$conn = db_connect();
								...
								$result=mysql_query("select DeptNo,DeptName from Departments where UniversityCode='$uncode' and  CollegeCode='$CollegeCode'");
	 			...
	 			<tr>
					...
					<td bordercolor="#003366" align="right" width="26%" bgcolor="#5A74A0" bordercolorlight="#003366" bordercolordark="#003366">
						<p align="center"><b>
							...
							//get ClassName
							$conn = db_connect();
							$resclass=mysql_query("select ClassName from ClassYear where ClassNo='$ClassNo'");
